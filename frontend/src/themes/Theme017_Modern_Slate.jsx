...
export default function Theme017_Modern_Slate({ invoice }) {
  ...
  return (
    ...
    <div className="max-w-4xl mx-auto p-12 bg-white rounded-2xl shadow-xl shadow-slate-100 border border-slate-50 relative overflow-hidden">
      {/* Watermark Lunas */}
      {isPaid && (
      ...
      <div className="flex justify-between items-start mb-12 border-b border-gray-100 pb-8">
        <div>
          <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-slate-400 to-slate-600 flex items-center justify-center text-white text-3xl font-bold shadow-lg mb-4">F</div>
        ...
        <div className="text-right">
          ...
          {!isPaid ? (
            <div className="mt-4 px-4 py-1.5 bg-slate-50 text-slate-700 rounded-full text-sm font-bold inline-block border border-slate-100">
              DUE: 14 DAYS
            </div>
          ) : (
      ...
      <div className="grid grid-cols-2 gap-8 mb-10">
        ...
        <div className={`p-6 rounded-xl text-white shadow-lg flex flex-col justify-center items-end ${isPaid ? 'bg-gradient-to-br from-emerald-500 to-emerald-600 shadow-emerald-200/50' : 'bg-gradient-to-br from-slate-500 to-slate-600 shadow-slate-200/50'}`}>
      ...
      <div className="flex flex-col sm:flex-row justify-between gap-8">
        <div className="w-full sm:w-1/2">
           <div className="bg-slate-50/50 p-5 rounded-xl border border-slate-100 mb-6">
              <p className="text-xs font-bold text-slate-600 uppercase mb-2">Terbilang (Say in Words)</p>
        ...
        <div className="w-full sm:w-1/3 space-y-3">
          ...
          <div className="flex justify-between text-sm text-slate-600 font-bold"><span>Tax / PPN (11%)</span><span>Rp {taxAmount.toLocaleString('id-ID')}</span></div>
          <div className="pt-4 border-t border-gray-200 flex justify-between items-center mt-2">
            ...
            <span className={`font-black text-2xl ${isPaid ? 'text-emerald-600' : 'text-transparent bg-clip-text bg-gradient-to-r from-slate-600 to-slate-800'}`}>
      ...
      <div className="mt-16 border-t border-gray-100 pt-8 flex justify-between items-end">
        ...
        <div className="text-center w-48 relative">
          ...
          {requiresMaterai && (
             <div className="absolute bottom-6 left-4 border-2 border-dashed border-slate-400 w-16 h-20 flex items-center justify-center text-[10px] text-slate-500 font-bold text-center bg-slate-50/50 transform rotate-[3deg]">
               E-MATERAI<br/>10000
             </div>
          )}
